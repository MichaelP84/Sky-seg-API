...
stub = modal.Stub("sky-seg")

seg_image = Image.debian_slim().copy_local_file(
    "isnetis.ckpt"
).pip_install(
    "numpy==1.24.3",
...
def process_mask(file: dict):
    ...
    # flatten mask
    flattened = mask.flatten().tolist()
    
    return {"mask": flattened, "height": mask.shape[0], "width": mask.shape[1]}
